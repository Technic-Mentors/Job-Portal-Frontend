...
export default function AddInterviewItem() {
  const [editorHtml, setEditorHtml] = useState("");
  const [slug, setSlug] = useState("");
  const {interviewCat} = useContext(InterviewCatContext)
  const {allInterviewEntries} = useContext(InterviewProvider)
  ...
  const resourceCreate = async (e) => {
    ...
    const res = await fetch(`${apiUrl}/api/interview/createInterview`, {
      method: "POST",
      headers: {
    ...
    if (res.ok) {
      Swal.fire({
              position: "center",
              icon: "success",
              title: "Interview item added successfully!",
